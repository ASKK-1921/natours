...
const createSendToken = (user, statusCode, res) => {
  ...
  // Define cooke options and send cookie
  const cookieOptions = {
    expires: new Date(
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!user || !(await user.correctPassword(password, user.password))) {
    return next(new AppError('Invalid user details', 401));
  }
...
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      ...
      res.locals.user = currentUser; // res.locals.XYZ makes XYZ available to pug templates
      return next();
    } catch (err) {
      return next();
    }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // Done on the userModel as a pre('save') function
